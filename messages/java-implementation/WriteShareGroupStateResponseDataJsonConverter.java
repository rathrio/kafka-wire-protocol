...
import static org.apache.kafka.common.message.WriteShareGroupStateResponseData.*;

public class WriteShareGroupStateResponseDataJsonConverter {
    public static WriteShareGroupStateResponseData read(JsonNode _node, short _version) {
        WriteShareGroupStateResponseData _object = new WriteShareGroupStateResponseData();
        JsonNode _resultsNode = _node.get("results");
        if (_resultsNode == null) {
            throw new RuntimeException("WriteShareGroupStateResponseData: unable to locate field 'results', which is mandatory in version " + _version);
        } else {
            if (!_resultsNode.isArray()) {
                throw new RuntimeException("WriteShareGroupStateResponseData expected a JSON array, but got " + _node.getNodeType());
            }
            ArrayList<WriteStateResult> _collection = new ArrayList<WriteStateResult>(_resultsNode.size());
            _object.results = _collection;
            for (JsonNode _element : _resultsNode) {
                _collection.add(WriteStateResultJsonConverter.read(_element, _version));
            }
        }
        return _object;
    }
    public static JsonNode write(WriteShareGroupStateResponseData _object, short _version, boolean _serializeRecords) {
        ObjectNode _node = new ObjectNode(JsonNodeFactory.instance);
        ArrayNode _resultsArray = new ArrayNode(JsonNodeFactory.instance);
        for (WriteStateResult _element : _object.results) {
            _resultsArray.add(WriteStateResultJsonConverter.write(_element, _version, _serializeRecords));
        }
        _node.set("results", _resultsArray);
        return _node;
    }
    public static JsonNode write(WriteShareGroupStateResponseData _object, short _version) {
    ...
    public static class WriteStateResultJsonConverter {
        public static WriteStateResult read(JsonNode _node, short _version) {
            WriteStateResult _object = new WriteStateResult();
            JsonNode _topicIdNode = _node.get("topicId");
            if (_topicIdNode == null) {
                throw new RuntimeException("WriteStateResult: unable to locate field 'topicId', which is mandatory in version " + _version);
            } else {
                if (!_topicIdNode.isTextual()) {
                    throw new RuntimeException("WriteStateResult expected a JSON string type, but got " + _node.getNodeType());
                }
                _object.topicId = Uuid.fromString(_topicIdNode.asText());
            }
            JsonNode _partitionsNode = _node.get("partitions");
            if (_partitionsNode == null) {
                throw new RuntimeException("WriteStateResult: unable to locate field 'partitions', which is mandatory in version " + _version);
            } else {
                if (!_partitionsNode.isArray()) {
                    throw new RuntimeException("WriteStateResult expected a JSON array, but got " + _node.getNodeType());
                }
        ...
        public static JsonNode write(WriteStateResult _object, short _version, boolean _serializeRecords) {
            ObjectNode _node = new ObjectNode(JsonNodeFactory.instance);
        ...
        public static JsonNode write(WriteStateResult _object, short _version) {
            return write(_object, _version, true);
        }
    }
}
