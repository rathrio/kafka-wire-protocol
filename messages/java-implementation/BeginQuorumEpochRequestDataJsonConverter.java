...
import static org.apache.kafka.common.message.BeginQuorumEpochRequestData.*;
...
public class BeginQuorumEpochRequestDataJsonConverter {
    public static BeginQuorumEpochRequestData read(JsonNode _node, short _version) {
        ...
        if (_leaderEndpointsNode == null) {
            if (_version >= 1) {
                ...
                _object.leaderEndpoints = new LeaderEndpointCollection(0);
            }
        } else {
            if (!_leaderEndpointsNode.isArray()) {
                throw new RuntimeException("BeginQuorumEpochRequestData expected a JSON array, but got " + _node.getNodeType());
            }
            LeaderEndpointCollection _collection = new LeaderEndpointCollection(_leaderEndpointsNode.size());
            _object.leaderEndpoints = _collection;
            for (JsonNode _element : _leaderEndpointsNode) {
                _collection.add(LeaderEndpointJsonConverter.read(_element, _version));
            }
        }
        return _object;
    }
    public static JsonNode write(BeginQuorumEpochRequestData _object, short _version, boolean _serializeRecords) {
        ...
        if (_version >= 1) {
            ...
            for (LeaderEndpoint _element : _object.leaderEndpoints) {
    ...
    public static class LeaderEndpointJsonConverter {
        public static LeaderEndpoint read(JsonNode _node, short _version) {
            LeaderEndpoint _object = new LeaderEndpoint();
            if (_version < 1) {
        ...
        public static JsonNode write(LeaderEndpoint _object, short _version, boolean _serializeRecords) {
            if (_version < 1) {
        ...
        public static JsonNode write(LeaderEndpoint _object, short _version) {
            return write(_object, _version, true);
        }
    }
