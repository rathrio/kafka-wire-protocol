...
public class ListOffsetsRequestDataJsonConverter {
    public static ListOffsetsRequestData read(JsonNode _node, short _version) {
        ...
        if (_timeoutMsNode == null) {
            if (_version >= 10) {
                throw new RuntimeException("ListOffsetsRequestData: unable to locate field 'timeoutMs', which is mandatory in version " + _version);
            } else {
                _object.timeoutMs = 0;
            }
        } else {
            _object.timeoutMs = MessageUtil.jsonNodeToInt(_timeoutMsNode, "ListOffsetsRequestData");
        }
        return _object;
    }
