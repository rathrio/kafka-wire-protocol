...
public class JoinGroupRequestDataJsonConverter {
    public static JoinGroupRequestData read(JsonNode _node, short _version) {
        ...
        if (_reasonNode == null) {
            if (_version >= 8) {
                throw new RuntimeException("JoinGroupRequestData: unable to locate field 'reason', which is mandatory in version " + _version);
            } else {
                _object.reason = null;
            }
        } else {
    ...
    public static JsonNode write(JoinGroupRequestData _object, short _version, boolean _serializeRecords) {
        ...
        if (_version >= 8) {
            if (_object.reason == null) {
                _node.set("reason", NullNode.instance);
            } else {
