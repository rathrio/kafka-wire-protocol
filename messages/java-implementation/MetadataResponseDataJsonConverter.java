...
public class MetadataResponseDataJsonConverter {
    public static MetadataResponseData read(JsonNode _node, short _version) {
        ...
        if (_errorCodeNode == null) {
            if (_version >= 13) {
                throw new RuntimeException("MetadataResponseData: unable to locate field 'errorCode', which is mandatory in version " + _version);
            } else {
                _object.errorCode = (short) 0;
            }
        } else {
            _object.errorCode = MessageUtil.jsonNodeToShort(_errorCodeNode, "MetadataResponseData");
        }
        return _object;
    }
