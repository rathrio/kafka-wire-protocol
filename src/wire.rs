...
use crate::protocol::{ApiVersionsRequest, MetadataRequest, MetadataResponse, Parser};
...
pub fn submit_api_versions_request(request: &ApiVersionsRequest) -> std::io::Result<()> {
    let mut stream = TcpStream::connect("localhost:9092")?;
    println!("Connected to server!");
